...
export const LoginScreen = ({ onLogin, showToast }) => {
  ...
  const handlePhoneCheck = async (e) => {
    ...
    try {
      ...
      if (snapshot.empty) {
        setErrorMessage("Nomor WhatsApp tidak terdaftar sebagai warga.");
      ...
      if (
        ...
        (data.email &&
          data.email.includes("@") &&
          !data.email.includes("@warga.bumiadipura.id"))
      ) {
      ...
      if (error.code === "auth/weak-password") {
        ...
        try {
          ...
          if (loginErr.code === "auth/invalid-credential") {
            msg =
              "Email sudah terdaftar dengan password berbeda. Silakan gunakan password yang sesuai atau login langsung.";
          } else {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      switch (error.code) {
        ...
        case "auth/too-many-requests":
          showToast(
            "Terlalu banyak permintaan reset. Coba lagi nanti.",
            "error",
          );
          break;
        case "auth/invalid-email":
    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-[#F5F7FA] relative overflow-hidden touch-none select-none overscroll-none">
      ...
      <div className="z-10 w-full max-w-sm animate-fade-in">
            ...
            >
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-gray-500">
                  Masukkan nomor WhatsApp yang terdaftar di RT
                </p>
              </div>
              ...
              {errorMessage && (
                <div className="flex items-start gap-2.5 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-2xl animate-fade-in">
                  ...
                  <div>
                    <p className="text-sm font-bold">Nomor Tidak Ditemukan</p>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-emerald-600 text-white py-3.5 rounded-2xl font-bold text-sm shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-2"
              >
                {isLoading ? (
          ...
          {mode === "login" && (
            ...
            >
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-emerald-600 text-white py-3.5 rounded-2xl font-bold text-sm shadow-lg shadow-emerald-200 hover:bg-emerald-700 hover:shadow-emerald-300 active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-4"
              >
                {isLoading ? (
                  <Loader2 className="w-5 h-5 animate-spin" />
                ) : (
          ...
          {mode === "activate" && (
            ...
            >
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-emerald-600 text-white py-3.5 rounded-2xl font-bold text-sm shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-4"
              >
                {isLoading ? (
                  <Loader2 className="w-5 h-5 animate-spin" />
                ) : (
              ...
              <button
                type="button"
                onClick={handleSwitchAccount}
                className="w-full text-gray-400 text-xs font-bold py-3 hover:text-gray-600"
              >
                Batal & Kembali
              </button>
            </form>
          ...
          {mode === "forgot_password" && (
            ...
            >
              ...
              <p className="text-[10px] text-yellow-600 mt-3 text-center italic px-4">
                * Cek Inbox atau folder <b>SPAM</b> jika email konfirmasi tidak
                muncul.
              </p>
        ...
        <p className="text-center text-[10px] text-gray-400 mt-8 font-medium">
          Bumi Adipura • Connected Community
        </p>
      </div>
    </div>
  );
